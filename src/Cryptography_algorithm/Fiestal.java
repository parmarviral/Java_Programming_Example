...
// tempor= Integer.toBinaryString(finalNum);
    ...
    void changeToString(ArrayList<Integer> arr2, PrintWriter out) {
        String finalTxt = "";
        String binString = "";
        String txt = String.format("%32s", Integer.toBinaryString(arr2.get(0))).replace(' ', '0') + ""
                + String.format("%32s", Integer.toBinaryString(arr2.get(1))).replace(' ', '0');
...
// long key = 0x1512323a1712323al;
        ...
        try {
            File encFile = new File("files/source.txt");
            File decFile = new File("files/cipher.txt");
            if (encFile.exists()) {
                encFile.delete();
...
//testing for avalanche effect
        ...
        System.out.println("using key 0x951Dl: "+String.format("%32s",
                Integer.toBinaryString(testArr.get(0))).replace(' ', '0') + "" + String.format("%32s",
                Integer.toBinaryString(testArr.get(1))).replace(' ', '0'));
        testArr = feistel.encrypt(0x46730l, 0x950Dl, keys);
        System.out.println("using key 0x950Dl: "+String.format("%32s",
                Integer.toBinaryString(testArr.get(0))).replace(' ', '0') + "" + String.format("%32s",
                Integer.toBinaryString(testArr.get(1))).replace(' ', '0'));
        testArr = feistel.encrypt(0x46730l, 0x123Al, keys);
        System.out.println("using key 0x123A: "+String.format("%32s",
                Integer.toBinaryString(testArr.get(0))).replace(' ', '0') + "" + String.format("%32s",
                Integer.toBinaryString(testArr.get(1))).replace(' ', '0'));
        testArr = feistel.encrypt(0x46730l, 0x133Al, keys);
        System.out.println("using key 0x133A: "+String.format("%32s",
                Integer.toBinaryString(testArr.get(0))).replace(' ', '0') + "" + String.format("%32s",
                Integer.toBinaryString(testArr.get(1))).replace(' ', '0'));
        testArr = feistel.encrypt(0x46730l, 0xFC28l, keys);
        System.out.println("using key 0xFC28l: "+String.format("%32s",
                Integer.toBinaryString(testArr.get(0))).replace(' ', '0') + "" + String.format("%32s",
                Integer.toBinaryString(testArr.get(1))).replace(' ', '0'));
        testArr = feistel.encrypt(0x46730l, 0xFC18l, keys);
        System.out.println("using key 0xFC18l: "+String.format("%32s",
                Integer.toBinaryString(testArr.get(0))).replace(' ', '0') + "" + String.format("%32s",
                Integer.toBinaryString(testArr.get(1))).replace(' ', '0'));
        testArr = feistel.encrypt(0x46730l, 0xB1DEl, keys);
        System.out.println("using key 0xBD1El: "+String.format("%32s",
                Integer.toBinaryString(testArr.get(0))).replace(' ', '0') + "" + String.format("%32s",
                Integer.toBinaryString(testArr.get(1))).replace(' ', '0'));
        testArr = feistel.encrypt(0x46730l, 0xBD2El, keys);
        System.out.println("using key 0xBD2El: "+String.format("%32s",
                Integer.toBinaryString(testArr.get(0))).replace(' ', '0') + "" + String.format("%32s",
                Integer.toBinaryString(testArr.get(1))).replace(' ', '0'));
        }
}
